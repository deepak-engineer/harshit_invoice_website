...
const AttendanceFlow = () => {
    ...
    useEffect(() => {
        if (!loading && employee && (!attendance || attendance.status === 'WORKING')) {
             startCamera();
        }
        return () => {
    ...
    const handleAction = async (type) => {
        ...
        try {
            ...
            const { match, distance } = compareDescriptors(descriptor, registeredDescriptor);
            ...
            if (type === 'CHECK IN') {
                ...
                toast.success(`Checked out! Worked ${Math.floor(res.data.minutes / 60)}h ${res.data.minutes % 60}m`);
            }
    ...
    return (
        <div className="max-w-md mx-auto space-y-6">
            <div className="text-center">
                ...
                <p className="text-slate-500">{new Date().toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                {isCompleted ? (
                    <div className="p-8 text-center flex flex-col items-center">
                        ...
                        <p className="text-slate-500 mt-2">You have completed your attendance for today.</p>
                    </div>
                ) : (
                ...
                {!isCompleted && (
                    <div className="p-6 text-center space-y-4">
                        <p className={`text-sm font-medium ${processing ? 'text-blue-600 animate-pulse' : 'text-slate-600'}`}>
                            {status}
                        </p>
                        ...
                        >
                            ...
                            <span>{processing ? 'Processing...' : (isCheckedIn ? 'CHECK OUT' : 'CHECK IN')}</span>
                        </button>
                    </div>
                )}
            </div>
